...
#SBATCH --job-name=audio_encode         # Job name
...
#SBATCH --output=/work/users/t/i/tis/V2Music/out/encode_job_%A_%a.out   # STDOUT file
#SBATCH --error=/work/users/t/i/tis/V2Music/err/encode_job_%A_%a.err    # STDERR file
...
# add -u for running python in unbuffered mode, otherwise python might buffer the output
# and you won't see the progress in real time until script finishes
uv run encode.py --start "${start}" --end "${end}"